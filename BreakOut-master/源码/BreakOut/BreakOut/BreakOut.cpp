...
//int DeathX[100];
vector<int> DeathX;
int offsetPos[4]={-10,-5,5,10};
int randI;
map<int,vector<int> > difPosMap;
map<int,int> difMoveK;
int indexArray[4]={2,2,2,2};
int index=2;
int moveK;
...
float dTime;
vector<vector<int> > blocks(30);
map<int,vector<vector<int> > > deathSetMap;
map<int,map<int,vector<vector<int> > > > difDeathSetMap;
int gameState;//游戏状态
int lastId, curId;
//int blocks[40][20];
...
void LoadSetup()
{
	...
	randI=rand()%(4);
	//randI=3;
	ballPos.cx = flatPos.cx+offsetPos[randI];
...
int caculateMinusPredict(int j){
	int predictXX;
	int lastFlatPos=difPosMap[randI][indexArray[randI]-1];
	int trade=lastFlatPos/40;
	predictXX=lastFlatPos-80*(trade-j);
	if(predictXX<0)
		predictXX=80*(trade-j)-lastFlatPos;
	return predictXX;
}

int caculatePlusPredict(int j){
	int predictXX;;
	int lastFlatPos=difPosMap[randI][indexArray[randI]-1];
	int trade=lastFlatPos/40;
	predictXX=lastFlatPos+80*(j-trade);
	if(predictXX>=800)
		predictXX=800-40*(j-trade);
	return predictXX;
}
void envluateE(int j){
	if(ballPos.dx<0){
		predictX = caculateMinusPredict(j);
	}
	else if(ballPos.dx>=0){
		predictX = caculatePlusPredict(j);
	}
}

//int move=difMoveK[randI];
//char buffer[128]={0};
//sprintf(buffer,"move==%d",move);
//MessageBox(0,buffer,NULL,0);

void predict(int j){
	if(difMoveK[randI] == indexArray[randI]&&difMoveK[randI]>=2){
		envluateE(j);
		vector<int> deathArray=difPosMap[randI];
		deathArray[indexArray[randI]]=predictX;	
		for(int i=indexArray[randI]+1;i<100;i++)
			deathArray[i]=deathArray[indexArray[randI]];
	difPosMap[randI]=deathArray;
	}
}
...
bool RenderFunc()
{
	...
	case GAME_MENU:
		if(sDone)
		{
			Qlogo.tex = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/logo1.png");
			InitQuad();
			sDone = false;
		}
		dTime += dt;
	...
	case GAME_READY:
		...
		{
			dTime = 0.0;
			back = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/gameback.png");
			pBack   = new hgeSprite(back, 0, 0, scrWidth, scrHeight);
			//pHge->Effect_Free(backSnd);
			sDone = false;
		}
		dTime += dt;
		if(dTime > 0.5f && dTime < 1.0f)
		{
			ready = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/go.png");
	...
	case GAME_START:
		 ...
		 ballPos.cx +ballHeight >= flatPos.cx - flatWidth && ballPos.cx <= flatPos.cx + flatWidth)
		{
			...
			difMoveK[randI]++;
			//int move=difMoveK[randI];
			//int move1 = indexArray[randI];
			//char buffer[128]={0};
			//sprintf(buffer,"move=%d,index=%d",move,move1);
			//MessageBox(0,buffer,NULL,0);
			if(predictX-flatWidth-ballHeight<=ballPos.cx&&predictX+flatWidth>=ballPos.cx&&difMoveK[randI]-1==indexArray[randI]){
				//int move=difMoveK[randI];
			//int move1 = indexArray[randI];
			//char buffer[128]={0};
			//sprintf(buffer,"index=%d",move1);
		//	MessageBox(0,buffer,NULL,0);
				Sleep(100);
				indexArray[randI]++;
			}
		...
		{
			//ballPos.dy = -ballPos.dy;
			//DeathX[index]=ballPos.cx;
			//index++;
			deathSetMap[moveK]=blocks;
	//		difDeathSetMap[randI]=deathSetMap;

			vector<int> deathArray=difPosMap[randI];
			deathArray[indexArray[randI]]=ballPos.cx;	
			indexArray[randI]++;
			for(int i=indexArray[randI];i<100;i++)
				deathArray[i]=deathArray[indexArray[randI]-1];
			difPosMap[randI]=deathArray;	
			gameState = GAME_READY;
			ready = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/ready.png");
			pReady  = new hgeSprite(ready,  0,   0, 256, 80);
			dTime = 0;
			sDone = false;
			gameAgain ++;
	...
	case GAME_OVER:
		...
		if(dTime > 1.5f)
		{
			back = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/menuback.png");
			pBack   = new hgeSprite(back, 0, 0, scrWidth, scrHeight);
			ready = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/ready.png");
			pReady  = new hgeSprite(ready,  0,   0, 256, 80);
			pGui->SetFocus(1);
...
void subPosCx(void *){
	while(flatPos.cx>difPosMap[randI][difMoveK[randI]]){
		flatPos.cx -= flatPos.dx;
		Sleep(500);
	}
}
...
bool FrameFunc()
{
	...
	if(gameState == GAME_START)
	{
		...
		int last=0;
		int now = getTime();
		if(flatPos.cx+flatPos.dx>=difPosMap[randI][difMoveK[randI]]){                           
			_beginthread(subPosCx,0,NULL);
		}
		else if(flatPos.cx+flatPos.dx<difPosMap[randI][difMoveK[randI]]){                       
			_beginthread(addPosCx,0,NULL);
		}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPSTR cmdLine, int nCmdShow)
{
	...
	for(int i=0;i<200;i++){
		deathSetMap[i]=blocks;
		DeathX.push_back(400);
	}
	for(int i=0;i<4;i++){
		difPosMap[i]=DeathX;
		difDeathSetMap[i]=deathSetMap;
	}
	...
	{
		Qlogo.tex = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/logo.png");
		mouse     = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/cursor.png");
		back      = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/menuback.png");
		rblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/rblock.png");
		gblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/gblock.png");
		bblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/bblock.png");
		wblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/wblock.png");
		yblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/yblock.png");
		oblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/oblock.png");
		pblock    = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/block/pblock.png");
		flat      = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/flat.png");
		ball      = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/particles.png");
		ready     = pHge->Texture_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/ready.png");

		menuSnd = pHge->Effect_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/menu.wav");
		backSnd = pHge->Effect_Load("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/back.mp3");
		...
		pHpar   = new hgeParticleSystem("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/demo.psi", pHps);
		pHpar->info.nEmission = 50;

		pBall->SetColor(0xFFFFFFFF);
		pBall->SetHotSpot(16,16);

		Fnt=new hgeFont("C:/Users/apple/Desktop/Breakout-master/BreakOut-master/程序/res/font1.fnt");
		pGui=new hgeGUI();
